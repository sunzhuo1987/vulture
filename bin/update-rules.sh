...
for i in `find $CONFDIR/security-rules/base_rules/ -name *.conf |grep -v bound_blocking`; do
	data=${data}"Include $i"`echo -e "\r\n"`
done
BASE_RULE=`sqlite3 $BASEDIR/www/db "SELECT count(*) from modsecurity where name='RULES: OWASP ModSecurity Core - BASE'"`
if [ $BASE_RULE = 0 ]; then
...
sqlite3 $BASEDIR/www/db "$CMD" || exit

data=""
for i in `find $CONFDIR/security-rules/optional_rules/ -name *.conf`; do
	data=${data}"Include $i"`echo -e "\r\n"`
done
OPT_RULE=`sqlite3 $BASEDIR/www/db "SELECT count(*) from modsecurity where name='RULES: OWASP ModSecurity Core - OPTIONAL'"`
if [ $BASE_RULE = 0 ]; then
...
SLR_RULE=`sqlite3 $BASEDIR/www/db "SELECT count(*) from modsecurity where name='RULES: OWASP ModSecurity Core - SpiderLabs Research'"`
if [ $SLR_RULE = 0 ]; then
...
EXP_RULE=`sqlite3 $BASEDIR/www/db "SELECT count(*) from modsecurity where name='RULES: OWASP ModSecurity Core - EXPERIMENTAL'"`
if [ $EXP_RULE = 0 ]; then
