...
sub handle_action {
    ...
    $log->debug($type);
    my @messages =
      qw/AUTH_SERVER_FAILURE ACCOUNT_LOCKED LOGIN_FAILED NEED_CHANGE_PASS ACL_FAILED/;
    ...
    if ($action) {
        ...
        if ( $action eq 'template' ) {
            ...
            #Get translations
            my $translations =
              Core::VultureUtils::get_translations( $r, $log, $dbh, $type );
            $html =
              Core::VultureUtils::get_style( $r, $log, $dbh, $app, $type,
                $title, {}, $translations );
        }
