...
class Intf(models.Model):
    ...
    #style = models.ForeignKey('Style',null=1)
	...
	for app in App.objects.filter(intf=self.id).all():
	    ...
	    for auth in auth_list:
	        if auth.auth_type == 'ssl':
	            f=open("%s/%s.ca" % (settings.PATH, str(self.id)+'-'+app.name), 'w')
	...
	for app in App.objects.filter(intf=self.id).all():
            ...
            for auth in auth_list:
                if auth.auth_type == 'ssl':
		    try:
                        f=open("%s/%s.ca" % (settings.PATH, str(self.id)+'-'+app.name), 'r')
	            except:
	                return True
...
class Components(models.Model):
    SSO_TYPES = (
        ...
        ('sso_forward_autologon', 'sso_forward_autologon'),
        ('sso_forward', 'sso_forward'),
        )
    desc = models.CharField(max_length=128)
    type = models.CharField(max_length=20, choices=SSO_TYPES)
    def __unicode__(self):
        return self.desc
    class Meta:
        db_table='components'
...
class Post(models.Model):
    ...
    field_var = models.CharField(max_length=50)
...
class App(models.Model):
    ...
    sso_forward = models.ManyToManyField(Components,null=1,blank=1,db_table='component_app')
    #style = models.ForeignKey('Style',null=1)
    def __str__(self):
        return self.name
    def get_absolute_url(self):
...
class MapURI(models.Model):
    ...
    uri_pattern = models.CharField(max_length=200, unique=1)
    type = models.CharField(max_length=20, choices=PLUGIN_TYPES)
    options = models.CharField(max_length=200, null=1)
    class Meta:
        db_table = 'map_uri'
...
class LDAP(models.Model):
        ...
        user_scope = models.IntegerField(choices=LDAP_SCOPE, blank=1, null=1)
        ...
        group_scope = models.IntegerField(choices=LDAP_SCOPE, blank=1, null=1)
...
class TPL(models.Model):
    TPL_TYPES = (
        ...
        ('NEED_CHANGE_PASS','NEED_CHANGE_PASS'),
        )
    name = models.CharField(max_length=200,unique=1)    
...
class Language(models.Model):
    ...
    class Meta:
        db_table = 'style_language'

class Style(models.Model):
    name = models.CharField(max_length=200,unique=1)
    css = models.ForeignKey(CSS)
    tpl = models.ManyToManyField(TPL, db_table='style_tpl_mapping')
    image = models.ForeignKey(Image)
    languages = models.ManyToManyField(Language,db_table='style_language_mapping')
    def get_absolute_url(self):
        return "/style_style/"
    def __unicode__(self):
        return self.name
