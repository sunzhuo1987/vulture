...
def vulture_create_object_auth(*args, **kwargs):
    return create_object(*args, **kwargs)
...
@login_required
def edit_component(request,object_id=None):
    form = ComponentsForm(request.POST or None,instance=object_id and Components.objects.get(id=object_id))
    form.post = Post.objects.order_by("field_desc").filter(component=object_id)    
    # Save new/edited component
    if request.method == 'POST' and form.is_valid():
        dataPosted = request.POST
        component = form.save()
        posts = Post.objects.filter(component=object_id)
        posts.delete()
        for data in dataPosted:
            ...
            if m != None:
                ...
                if desc and var and type:
                    instance = Post(component=component, field_desc = desc, field_var = var, field_type = type, field_value = dataPosted['field_value-' + id], field_prefix = dataPosted['field_prefix-' + id], field_suffix = dataPosted['field_suffix-' + id])
                    instance.save()
...
@login_required
def edit_post(request,object_id=None):
    form = PostForm(request.POST or None,instance=object_id and Post.objects.get(id=object_id))
    # Save new/edited student
    if request.method == 'POST' and form.is_valid():
        form.save()   
        #app = form.save(commit=False)
        #app.security = form.cleaned_data['security']
        #app.save()
        #form.save_m2m()

        return HttpResponseRedirect('/components/')

    return render_to_response('vulture/post_form.html', {'form': form, 'user' : request.user})
...
def edit_acl(request,object_id=None):
    ...
    if object_id:
        ...
        if acl.auth.auth_type == "LDAP" :
            groups_ok = acl.auth.getAuth().group_ok()
    ...
    if request.method == 'POST' and form.is_valid():
        ...
        for value in dataPosted.getlist('in_group[]'):
            try:
                group = GroupOK.objects.get(user=value)
                acl.groups_ok.add(user)
    ...
    return render_to_response('vulture/acl_form.html', {'form': form, 'user' : request.user, 'users_ok' : users_ok, 'users_ko' : users_ko})
