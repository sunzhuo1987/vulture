...
sub checkACL {
    ...
    my $query =
"SELECT count(*) FROM userok,acl_userok WHERE acl_userok.acl_id = ? AND userok.user=? AND acl_userok.userok_id=userok.id";
    $log->debug($query);

    if (
        $dbh->selectrow_array(
            ( $query, undef, $app->{'acl'}->{'id'}, $user )
        )
      )
    {
        ...
        #Get LDAP groups
        my $sql =
"SELECT `group` FROM groupok, acl_groupok WHERE acl_groupok.acl_id = ? AND groupok.id = acl_groupok.groupok_id";
        ...
        $mesg = $ldap->search(
            base => $ldap_group_ou ? $ldap_group_ou : $ldap_base_dn,
