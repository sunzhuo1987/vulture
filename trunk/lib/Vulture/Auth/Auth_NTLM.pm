...
use Apache2::Const -compile =>
  qw(HTTP_UNAUTHORIZED HTTP_INTERNAL_SERVER_ERROR DECLINED HTTP_FORBIDDEN OK FORBIDDEN);

use strict;
use vars
  qw{$cache $VERSION %msgflags1 %msgflags2 %msgflags3 %invflags1 %invflags2 %invflags3 $addr $port $debug};
$cache = undef;
use MIME::Base64 ();
use Socket;
use mod_perl2;
...
sub get_msg1 {
    ...
    my (
        $protocol, $type,    $zero, $flags1,   $flags2, $zero2,    $dom_len,
        $x1,       $dom_off, $x2,   $host_len, $x3,     $host_off, $x4
    ) = unpack( 'Z8Ca3CCa2vvvvvvvv', $data );
    ...
    $log->debug(
"Auth_NTLM: get_msg1: domain == $domain host == $host accept_unicode == $accept_unicode"
    );
    ...
    if ($debug) {
        ...
        $log->debug(
"[$$] AuthenNTLM: protocol=$protocol, type=$type, flags1=$flags1($flag1str), "
...
sub set_msg2 {
    ...
    my $charencoding =
        $accept_unicode
      ? $invflags1{NEGOTIATE_UNICODE}
      : $invflags1{NEGOTIATE_OEM};
    my $flags2 = $invflags2{NEGOTIATE_ALWAYS_SIGN} | $invflags2{NEGOTIATE_NTLM};
    my $data   = pack( 'Z8Ca7vvCCa2a8a8',
        'NTLMSSP', 2, '', 40, 0, $charencoding, $flags2, '', $nonce, '' );
    my $header = 'NTLM ' . MIME::Base64::encode( $data, '' );

    $log->debug(
"Auth_NTLM: set_msg2 charencoding = $charencoding flags2 = $flags2 nonce=$nonce Send header: $header:'NTLM' ..."
    );
...
sub get_msg3 {
    ...
    my (
        $protocol, $type,    $zero,     $lm_len, $l1,
        $lm_off,   $nt_len,  $l3,       $nt_off, $dom_len,
        $x1,       $dom_off, $user_len, $x3,     $user_off,
        $host_len, $x5,      $host_off, $msg_len
    ...
    $log->debug(
"Auth_NTLM: get_msg3 protocol=$protocol, type=$type, user=$user, host=$host, domain=$userdomain, msg_len=$msg_len, user_nthash=$usernthash"
    );
...
sub checkAuth {
    ...
    if ( $t == 1 ) {
        ...
        ( $type, $username, $usernthash, $userdomain ) =
          get_msg3( $r, $log, $data, $domain );
    }
    ...
    if ( $type == 1 ) {
        ...
        $log->debug(
'Auth_NTLM: verify handle = 1 smbhandle == $self->{smbhandle} nonce == $nonce '
        );
        ...
        $log->debug(
"Auth_NTLM: Authen::Smb::Valid_User_Auth --> Call with smbhandle=$self->{smbhandle}, username=$username, nt_hash=$usernthash, 1, domain=$userdomain"
        );
        my $rc =
          Authen::Smb::Valid_User_Auth( $self->{smbhandle}, $username,
            $usernthash, 1, $userdomain );
        ...
        Authen::Smb::Valid_User_Disconnect( $self->{smbhandle} )
          if ( $self->{smbhandle} );
        ...
        if ( $rc == &Authen::Smb::NTV_LOGON_ERROR ) {
            $log->debug(
"Auth_NTLM: Wrong password/user (rc=$rc/$errno/$smberr): $userdomain\\$username for "
                  . $r->uri );
            my $hdr = $r->err_headers_out;
            $hdr->add( 'WWW-Authenticate', 'NTLM' );
            return Apache2::Const::HTTP_UNAUTHORIZED;
        ...
        if ($rc) {
            $log->debug( "Auth_NTLM: SMB Server error $rc/$errno/$smberr for "
                  . $r->uri );
            my $hdr = $r->err_headers_out;
            $hdr->add( 'WWW-Authenticate', 'NTLM' );
            return Apache2::Const::HTTP_UNAUTHORIZED;
        ...
        $self->{lock} = undef;
        $log->debug( 'Auth_NTLM: Bad NTLM Authorization Header type $type for '
              . $r->uri );
        return Apache2::Const::HTTP_UNAUTHORIZED;
    }
...
sub lock {
    ...
    $self->{sem} = new IPC::Semaphore( $key, 1, IPC_CREAT | S_IRWXU )
      or die "Cannot create semaphore with key $key ($!)";

    $self->{sem}->op( 0, 0, SEM_UNDO, 0, 1, SEM_UNDO );
