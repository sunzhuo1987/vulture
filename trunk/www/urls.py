...
urlpatterns = patterns('',
  ...
  (r'^sso/$',                                 'vulture.views.vulture_object_list_adm', dict({'queryset': Components.objects.all()}, allow_empty=1)),
  (r'^sso/(?P<object_id>\d+)/$',              'vulture.views.edit_component'),
  (r'^sso/new/$',                             'vulture.views.edit_component'),
  (r'^sso/(?P<object_id>\d+)/del/$',          'vulture.views.vulture_delete_object_adm', dict({'model' : Components}, post_delete_redirect='/sso/')),
  ...
  (r'^ldap/$',                                'vulture.views.vulture_object_list', {'queryset': LDAP.objects.all()}),
  (r'^(?P<url>ldap)/(?P<object_id>\d+)/edit/$','vulture.views.edit_auth'),
  (r'^(?P<url>ldap)/new/$',                   'vulture.views.edit_auth'),
  (r'^(?P<url>ldap)/(?P<object_id>\d+)/del/$','vulture.views.delete_auth'),
