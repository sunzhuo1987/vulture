...
sub forward {
    ...
    my (%session_SSO);
    Core::VultureUtils::session( \%session_SSO, $app->{timeout},
    ...
    my $sql =
"SELECT field.field_var, field.field_mapped, field.field_type, field.field_encrypted, field.field_value, field.field_prefix, field.field_suffix, field.field_desc FROM field, sso, app WHERE field.sso_id = sso.id AND sso.id = app.sso_forward_id AND app.id=? AND field.field_type != 'autologon_user' AND field.field_type != 'autologon_password' AND field.field_type != 'hidden'  AND field.field_type != 'cookie' ORDER BY field.field_type DESC";
    $log->debug($sql);
    ...
    foreach my $field (@fields) {
        my ( $var, $mapped, $type, $need_encryption, $default_value, $prefix,
            $suffix, $desc )
          = @$field;

        #Set form
        if (    $type ne 'autologon_user'
            and $type ne 'autologon_password'
            and $type ne "hidden" )
        {
            $form .=
"<tr><td>$desc</td><td><input type=\"$type\" name=\"$var\" value=\"$default_value\"></td></tr>";
        }
        else {
            next;
        }
    ...
    my $html = Core::VultureUtils::get_style(
        $r, $log, $dbh, $app, 'LEARNING',
    ...
    if ( not $req->param('vulture_learning') ) {
        ...
        SSO::ProfileManager::set_profile(
            $r, $log, $dbh, $app, $user, @fields
        )
      )
    {
        $r->content_type('text/html');
